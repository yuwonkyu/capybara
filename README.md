...
- 공지사항 게시판 글쓰기는 관리자(길드마스터·부마스터) 전용입니다. 자유게시판·공략·사냥·거래 게시판은 로그인한 누구나 쓸 수 있습니다.
...
- 길마용 **집계 표(아이디 / 투자횟수 / 회원등급)** 와 **엑셀 다운로드**(관리자 전용, 카피+카피랜드 2개 시트)를 제공합니다.
- **직접 등록은 길드마스터·부마스터만** 가능합니다. 디스코드에 못 올린 길드원의 투자를 대신 입력하는 용도로, **투자자 닉네임을 직접 입력**합니다(로그인한 관리자 본인 명의로 등록되지 않습니다).
- 기부 기록 삭제는 관리자, 또는 본인 로그인 계정으로 등록한 기록이면 본인도 가능합니다. (관리자가 대신 등록한 기록·디스코드 동기화 기록은 특정 로그인 계정에 연결되지 않아 관리자만 삭제할 수 있습니다.)
...
- 회원등급 표시 우선순위: ① `members.discord_user_id` 로 연결된 디스코드 계정 ② 로그인 계정(`user_id`) ③ **닉네임 텍스트가 사이트 회원의 닉네임과 정확히 같으면** 그 등급을 표시. 셋 다 해당 없으면 `미연동`으로 표시됩니다. (닉네임 매칭은 표시용일 뿐 실제 권한 부여는 아닙니다.)

### 회원 등급 & 관리자 페이지

- 회원 등급은 길드마스터 · 부마스터 · 카피바라 · 카피 · 새싹 5단계입니다. 로그인하면 자동으로 새싹으로 등록됩니다.
- 관리자 권한(공지 작성·글 관리)은 길드마스터·부마스터가 가집니다.
- **길드마스터**는 헤더의 `관리자` 버튼(`/admin`)에서 전체 길드원 목록을 보고 등급 변경·추방을 할 수 있습니다.
- **카피바라 · 카피 · 새싹**은 누적 투자 금액(길드 스킬 투자 기부액, 카피+카피랜드 합산)에 따라 **자동으로 산정**됩니다.
  - 5,000만 메소 이상 → 카피바라
  - 1,000만 메소 이상 → 카피
  - 그 미만 → 새싹
  - 기부를 등록·수정·삭제하거나 디스코드 동기화를 하면 즉시 재계산됩니다. 단, 디스코드 기록은 `members.discord_user_id`로 사이트 계정과 연결되어 있어야 등급에 반영됩니다.
  - 길드마스터·부마스터는 자동 조정 대상이 아니며, `/admin`에서 수동으로만 바뀝니다.
