  ...
  return (
    <main className="relative mx-auto min-h-screen w-full max-w-6xl px-4 pb-12 pt-6 sm:px-8">
      ...
      <header className="mb-5 flex flex-col gap-3 rounded-3xl border border-white/50 bg-cream/70 p-5 shadow-candy backdrop-blur md:flex-row md:items-center md:justify-between">
        <div>
          <p className="font-display text-4xl text-[#ff5f7c]">
            친절한 카피바라씨
          </p>
          <p className="font-body text-sm text-[#315f73]">
            Maple Planet Guild Home
          </p>
        </div>
        <a
          href="#join"
      ...
      <section className="mb-5 overflow-hidden rounded-3xl border border-[#ffd7df] bg-gradient-to-br from-[#fff8de] via-[#ffe9ef] to-[#eaf8ff] shadow-candy">
        <div className="grid gap-6 p-6 lg:grid-cols-[1.1fr_0.9fr] lg:items-center lg:p-8">
          <div>
            <p className="font-body mb-2 inline-block rounded-full bg-white px-3 py-1 text-xs text-[#e35d78]">
              길드 전용 페이지
            </p>
            <h1 className="font-display text-5xl leading-tight text-[#f86384] sm:text-6xl">
              같이 성장하는 카피 길드
            </h1>
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              {highlights.map((item) => (
                <span
                  key={item}
                  className="rounded-full bg-white px-4 py-2 font-body text-sm text-[#365f70] shadow-sm"
                >
                  {item}
                </span>
              ))}
      ...
      <section className="grid gap-4 md:grid-cols-2">
        ...
        <article className="cute-card">
          ...
          <p className="font-body mb-3 text-sm text-[#6a8693]">
          ...
          <ul className="space-y-2">
            {schedules.map((schedule) => (
              <li
                ...
                className="font-body grid grid-cols-[40px_1fr] gap-2 text-[#365f70]"
              >
        ...
        <article id="join" className="cute-card animate-floaty">
          ...
          <p className="font-body text-[#365f70]">
            ...
            싶은 분이라면 누구나 환영해요. 방송은 치지직과 유튜브에서 만나볼 수
            있어요.
          </p>
          <div className="mt-4 flex flex-wrap gap-2">
            <a
              href="https://www.youtube.com/@kindcapybara"
              className="font-body inline-flex items-center rounded-xl bg-white px-4 py-2 text-[#365f70] shadow-sm transition hover:-translate-y-0.5"
            >
              유튜브 채널 보기
            </a>
